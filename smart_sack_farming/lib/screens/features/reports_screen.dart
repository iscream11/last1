...
class _ReportsScreenState extends State<ReportsScreen>
  ...
  final List<CalamityReport> _calamityReports = [
    CalamityReport(
      id: '1',
      type: 'Flood',
      description:
          'Heavy rainfall caused waterlogging in the southern field. Crops submerged for 3 days.',
      severity: 'high',
      dateOccurred: DateTime.now().subtract(const Duration(days: 5)),
    ...
    CalamityReport(
      ...
      type: 'Storm',
      description:
          'Heavy storm with strong winds damaged crop stalks and knocked down plants.',
      severity: 'medium',
      dateOccurred: DateTime.now().subtract(const Duration(days: 2)),
      dateReported: DateTime.now().subtract(const Duration(days: 2)),
      farmerId: 'farmer_002',
      farmerName: 'Priya Singh',
      affectedArea: 3.2,
      affectedCrops: ['Maize', 'Cotton'],
      status: 'reported',
      imageUrl: '⛈️',
    ),
  ];

  final List<ProductionReport> _productionReports = [
    ProductionReport(
      id: '1',
      cropType: 'Rice',
      area: 4.5,
      ...
      totalYield: 202.5,
      qualityRating: 4.5,
      notes: 'Good yield with minimal pest damage. Premium quality grains.',
    ...
    ProductionReport(
      ...
      cropType: 'Wheat',
      area: 3.0,
      plantingDate: DateTime.now().subtract(const Duration(days: 140)),
      harvestDate: DateTime.now().subtract(const Duration(days: 20)),
      yieldPerAcre: 38,
      totalYield: 114,
      qualityRating: 4.0,
      notes: 'Moderate yield. Some water stress in later stages.',
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                setState(() {
                  _calamityReports.removeAt(index);
                });
                ScaffoldMessenger.of(context).showSnackBar(
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                ...
                final newReport = CalamityReport(
                  ...
                  status: 'reported',
                  imageUrl: _getIconForCalamity(type ?? 'Flood'),
                );
                setState(() {
                  _calamityReports.insert(0, newReport);
                });
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Calamity reported successfully')),
                );
              }
            },
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                formKey.currentState!.save();
                final newReport = ProductionReport(
                  id: DateTime.now().millisecondsSinceEpoch.toString(),
                  cropType: cropType ?? '',
                  area: area ?? 0,
                ...
                setState(() {
                  _productionReports.insert(0, newReport);
                });
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
